...
def get_threat_color_and_label(cls_name: str, conf: float, speed: float) -> Tuple[Tuple[int, int, int], str]:
    """Get color and label based on detected class and behavior."""
    cls_name_lower = cls_name.lower()
    
    # Simple threat detection based on movement and confidence
    if cls_name_lower == 'person':
        if speed > 20 and conf > 0.7:  # Fast moving, high confidence
            return (0, 0, 255), "ROBBERY"  # Red for threats
        elif speed < 5 and conf > 0.6:  # Slow moving, could be victim
            return (0, 255, 0), "VICTIM"  # Green for victims
        else:
            return (0, 255, 255), "PERSON"  # Cyan for normal
    elif cls_name_lower in ['car', 'truck', 'bus']:
        return (255, 255, 0), "VEHICLE"  # Yellow for vehicles
    else:
        ...
        while True:
            ...
            if r is not None and r.boxes is not None and r.boxes.id is not None:
                ...
                for xyxy, conf, tid, cls in zip(boxes_xyxy, scores, ids, classes_iter):
                    ...
                    # Check if moving enough
                    if only_moving and speed < speed_threshold:
                        continue
                    
                    # Get class-based color and label
                    if cls is not None:
                        cls_name = names.get(int(cls), "obj") if isinstance(names, dict) else "obj"
                        color, threat_label = get_threat_color_and_label(cls_name, float(conf), speed)
                        
                        if only_moving:
                            label = f"{threat_label} {tid} • {int(speed):d}px/f"
                        else:
                            label = f"{threat_label} {tid}"
                    else:
